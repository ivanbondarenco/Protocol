...
export const ProfileModal = ({ isOpen, onClose }: ProfileModalProps) => {
    const { bioData, updateBioData, updateGymStats, recalculateTargets, theme } = useProtocolStore();
    ...
    return (
        <motion.div
            ...
            className={`fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md ${isCyberpunk ? 'bg-black/90' : 'bg-black/80'}`}
            onClick={onClose}
        >
            <motion.div
                initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
                className={`w-full max-w-sm overflow-hidden rounded-2xl border p-6 relative shadow-2xl
                    ${isCyberpunk ? 'bg-black border-accent-neon shadow-[0_0_30px_rgba(0,242,255,0.2)]' : 'bg-neutral-900 border-white/10'}
                `}
                onClick={e => e.stopPropagation()}
            >
                <div className="flex justify-between items-center mb-6">
                    <h3 className={`font-bold uppercase tracking-widest flex items-center gap-2 ${isCyberpunk ? 'text-accent-neon' : 'text-white'}`}>
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex flex-col items-center gap-4">
                        <div className={`relative w-24 h-24 rounded-full overflow-hidden border-2 
                            ${isCyberpunk ? 'border-accent-neon shadow-[0_0_15px_rgba(0,242,255,0.4)]' : 'border-white/20'}
                        `}>
                            {avatarUrl ? (
                        ...
                        <div className="w-full">
                            ...
                            <input
                                ...
                                className={`w-full bg-black/50 border p-2 text-xs text-white outline-none rounded
                                    ${isCyberpunk ? 'border-accent-neon/30 focus:border-accent-neon' : 'border-white/10 focus:border-white/30'}
                                `}
                                placeholder="https://..."
                            />
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input
                                ...
                                className={`w-full bg-black/50 border p-2 text-center text-white outline-none rounded font-mono
                                    ${isCyberpunk ? 'border-accent-neon/30 focus:border-accent-neon' : 'border-white/10 focus:border-white/30'}
                                `}
                            />
                        </div>
                    ...
                    <div>
                        <div className="flex justify-between items-end mb-2">
                            ...
                            <span className={`font-mono font-bold ${isCyberpunk ? 'text-accent-neon' : 'text-white'}`}>{activity}x</span>
                        </div>
                        <input
                            ...
                            className={`w-full h-2 rounded-lg appearance-none cursor-pointer bg-white/10
                                ...
                                ${isCyberpunk ? '[&::-webkit-slider-thumb]:bg-accent-neon' : '[&::-webkit-slider-thumb]:bg-white'}
                            `}
                        />
                    ...
                    <button
                        onClick={handleSave}
                        className={`w-full py-3 font-bold uppercase tracking-widest text-xs flex items-center justify-center gap-2 rounded transition-all mt-4
                            ${isCyberpunk
                                ? 'bg-accent-neon text-black hover:bg-white shadow-[0_0_15px_rgba(0,242,255,0.4)]'
